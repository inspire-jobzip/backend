...
| 구분 | 내용 |
|---|---|
| 주요업무 | MySQL 기반 대용량데이터 처리 및 관리 광고주, 광고계정, 매체별 실적 데이터집계 및통계 50! 쿼리 튜닝및 성능개선 인덱스 설계, 데이터 수집, 가공, 적재 프로세스 관리 배치성 데이터 처리 및 재처리 구조 관리 사내 데이터 시스템의 안정성 및 처리 속도 개선 NCP 기반서버및08운영 |
| 자격요건 | 관련경력 3년이상 대용량 데이터 처리 및 SQL 쿼리 작성 및 튜닝 가능자 인덱스 설계 및 실행계획 분석 경험자 Linux A] 712 사용 가능자 서버로그확인및 기본장애 분석가능자 |
| 우대사항 | MySQL DBA SEi= DBA 성향의 업무 경험 대용량테이블 성능 개선 경험 배치 처리, 스케줄링, 데이터 재처리 구조 경험 광고, 커머스, CRM, ERP, 정산, 통계 시스템 데이터처리 경험 NCP Cloud DB for MySQL 2% 2g Python, PHP, Node js Z 1개이상사용경험 관리자 페이지, 통계 대시보드, 리포트 시스템 개발 경험 데이터 정합성 검증 및 오류 데이터 보정 경험 사내 업무시스템 운영 및 개선 경험 사용환경 Database: MySQL Infra: NCP 0S: Linux Data: 2115 데이터, 광고계정 데이터, 매체별 실적 데이터, 매출 통계 데이터 개발/운영: SQL, 배치 처리, 데이터 집계, 서버 로그 확인, DB 성능 개선 |
| 근무조건 | [고용형태] 정규직 [근무부서] 개발팀 [근무지] 서울특별시 금천구 가산디지털2로166 3층(에이스1<1타워} [경력] 3년이상 [B01] 경력 및 역량에 따라 협의(접수 시 희망연봉 기재 필수) [근무시간] 09:30~17.30(주 5일, 35시간) 제출서류 이력서, 경력기술서, 포트폴리오(선택), 희망연봉 기재필수 경력기술서에는 아래 경험을 SHOR 작성해 주시면 좋습니다.ㆍ MySQL 쿼리 튜닝경험 + 대용량데이터처리경험 - 인덱스설계경험 * 통계/집계데이터처리 경험 ＊ NCP 또는 클라우드 서버운영경험<br>ㆍ 장애분석및성능개선사례 |
| 채용절차 | 서류전형 > 실무면접 > 최종합격 ee |',
  NULL,
...
비케이브(B.CAVE)는 브랜드를 넘어 문화를 만드는 패션기업 입니다.

단순히 옷을 만드는 회사가 아니라, 브랜드를 통해 사람과 일상, 문화를 잇는 여정을 설계해 갑니다.

커버낫(COVERNAT), 와키윌리(Wacky WiLLy), 리(LEE) 등 다양한 캐주얼 브랜드를 전개하며 브랜드의 임팩트를 세계 시장으로 확장시키고, 감도 높은 시선과 유연한 실행력으로 빠르게 변화하는 트렌드를 이끌어가고 있습니다.

패션으로 일상과 문화를 연결하는 여정에 합류해 주세요. 더 많은 이들의 일상에 영감과 변화를 전할 당신을 기다립니다.

AX팀을 소개합니다.
AX팀은 사내 다양한 업무 영역에서 도출된 AI/자동화 과제를 실제 서비스로 전환하고, 조직 전반에서 활용 가능한 구조로 만드는 역할을 합니다. 단순한 PoC 수준을 넘어, 운영 가능한 플랫폼과 데이터 인프라를 구축하여 디자인·상품기획·마케팅 등 주요 기능 조직의 업무 방식을 재설계하고 있습니다.

2. 모집부문: Data Engineer (데이터 엔지니어)
[담당업무]
Snowflake 기반 사내 데이터 플랫폼(DW/DM) 구축 및 운영
...
중국·동남아 생산처 직접 소싱 또는 해외 공장 커뮤니케이션 경험

ERP 등 상품 관리 시스템 활용 경험

트렌드 리딩 및 신규 협력업체 개발 경험

3. B.CAVE BENEFIT (복리후생 요약)
복지포인트
...
4. 복리후생 상세
지원금/보험: 건강검진, 각종 경조사 지원, 복지카드/포인트

급여제도: 퇴직연금, 퇴직금, 연차수당, 직책수당, 4대 보험

선물: 명절선물/귀향비, 웰컴키트 지급, 생일자 조기퇴근

교육/생활: 도서구입비지원, 저녁식사 제공, 사내동호회 운영, 간식 제공, 음료제공(차, 커피)

근무 환경: 카페테리아, 자회사 제품할인

조직문화: 수평적 조직문화, 자유복장, 자유로운 연차사용

출퇴근: 주차장제공

리프레시: 연차, 반차, Refresh휴가, 근로자의날 휴무, 산전 후 휴가, 육아휴직

5. 채용 프로세스 및 근무 조건
[채용 프로세스]
서류전형 ➡️ 인터뷰전형 ➡️ 처우협의 ➡️ 합격안내 ➡️ 합류

서류합격자에 한해 개별연락 드립니다.

전형 절차는 지원자의 이력사항에 따라 변경될 수 있습니다.

입지원 서류에 허위사실이 발견될 경우, 채용확정 이후라도 채용이 취소될 수 있습니다.
...
6. 제출서류 및 지원 안내
[제출서류]
이력서 (자유양식) *개인정보(연봉정보, 가족관계, 주민번호 등) 제외
...
지원방법: 당사 채용 홈페이지 온라인 접수 또는 사람인 직접 지원

[참고사항]
본 공고는 채용 시 조기에 마감될 수 있는 수시채용으로 진행됩니다.

보훈 및 장애인 등 취업보호대상자는 관계법에 의거 우대합니다.

채용 관련 궁금하신 부분은 채용담당자 hr@bcave.co.kr 로 문의주세요.

이력서 제출 시 개인정보(연봉정보, 가족관계, 주민번호 등) 제외 바랍니다.',
  NULL,
...
경력/고용형태: 경력무관 · 정규직
학력: 학력무관
채용공고 상세 2026 하반기 데이터팀 모집 공고 데이터 사이언티스트 모집 공고(정규직) 모집분야 데이터 사이언티스트 (0명)
데이터 사이언티스트(0명)
...
• Databricks(Notebook)기반 AI/ML 모델 개발 및 배포(모델 재학습, 고도화 등 AI 모델에 대한 자동화 및 고도화 포함)

• 건설·산업 현장 데이터 수집·정제·변환(ETL/ELT) 및 품질 관리
...
• 엑셀 및 Power BI 활용 가능자 (DAX 및 M Query)

• 팀 협업 및 커뮤니케이션 역량 (매우 중요)
...
• 데이터 파이프라인·ETL 프로젝트 경험 (개인·학습 프로젝트 포함)

• Power BI·Power Platform 등 데이터 도구 활용 경험

• 건설·제조·산업 도메인 데이터 경험 또는 관심
...
• 근무요일/시간 : 주 5일 (월~금) 08:00~17:30 / (점심시간 : 11:30 ~ 13:00)

🎁 복지 및 혜택

• 교육/생활 : 워크샵, 신규 입사자 교육(OJT), 직무능력향상교육, 식비 지원, 음료제공(차,커피)
...
접수기간 : 2026-07-08 16시 ~ 2026-08-07 24시
제출서류 : 사람인 온라인 이력서, *필수첨부* - 자기소개서*, 이력서*, 포트폴리오(선택사항)
접수방법 : 사람인 입사지원
